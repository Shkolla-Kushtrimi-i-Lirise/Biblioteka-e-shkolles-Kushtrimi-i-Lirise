        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-12">
          <div className="space-y-6">
            <label className="block">
              <span className="text-[10px] uppercase tracking-widest font-bold text-ink/40 ml-1">Kopertina</span>
            ...
            <tbody className="divide-y divide-primary/5">
              {loginHistory.length > 0 ? loginHistory.map(session => (
                <tr key={session.id}>
                  ...
                  <td className="py-4 text-ink/60 text-right italic">{session.timestamp}</td>
                </tr>
              )) : (
                <tr>
                  <td colSpan={2} className="py-8 text-center text-ink/40 italic">Asnjë kyçje e regjistruar deri tani.</td>
                </tr>
              )}
            </tbody>
          </table>
